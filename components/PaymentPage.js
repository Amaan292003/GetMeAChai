            ...
            <div className="info flex justify-center items-center my-24 mb-32 flex-col gap-2">
                ...
                <div className="payment flex gap-3 w-[80%] mt-11 flex-col md:flex-row">
                    <div className="supporters w-full md:w-1/2 bg-slate-900 rounded-lg text-white px-2 md:p-10">
                        ...
                        <ul className='mx-2 md:mx-5 text-sm md:text-lg'>
                            {payments.length == 0 && <li>No payments yet</li>}
                            {payments.map((p, i) => {
                                return <li key={i} className='my-4 flex gap-2 items-center'>
                                    <img width={33} src="avatar.gif" alt="user avatar" />
                                    <span>
                                        {p.name} donated <span className='md:font-bold '>₹{p.amount}</span> with a message &quot;{p.message}&quot;
                                    </span>
                                </li>
                            })}
                    ...
                    <div className="makePayment w-full md:w-1/2 bg-slate-900 rounded-lg text-white px-2 md:p-10">
                        ...
                        <div className='flex flex-col md:flex-row gap-2 mt-5'>
                            <button className='bg-slate-800 p-3 disabled:bg-slate-600 disabled:from-purple-100 rounded-lg'disabled={paymentform.name?.length < 3 || paymentform.message?.length < 4}  onClick={() => pay(1000)}>Pay ₹10</button>
                            <button className='bg-slate-800 p-3 disabled:bg-slate-600 disabled:from-purple-100 rounded-lg'disabled={paymentform.name?.length < 3 || paymentform.message?.length < 4}  onClick={() => pay(2000)}>Pay ₹20</button>
                            <button className='bg-slate-800 p-3 disabled:bg-slate-600 disabled:from-purple-100 rounded-lg'disabled={paymentform.name?.length < 3 || paymentform.message?.length < 4}  onClick={() => pay(3000)}>Pay ₹30</button>
                        </div>
                    </div>
                </div>
            </div>
